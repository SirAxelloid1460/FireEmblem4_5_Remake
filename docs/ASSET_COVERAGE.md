# Cobertura de assets — personajes y clases

> Generado por `tools/asset_coverage.py`. Regenera con `python tools/asset_coverage.py`.

> Leyenda: ✅ = tenemos · ⬜ = falta · ⚠️ = el asset existe pero su nombre no coincide con `combat_anim_nid` (el juego NO lo encuentra) · — = N/A


## Resumen

- Clases: **66** · con map sprite: **66** · con combat anim genérica: **5**
- Personajes: **64** · con retrato: **59**
- Carpetas de combat anim: **19**


## Clases — map sprites y combat anim (genérico / por género)

| Clase | Tier | Map stand | Map move | Anim Generic | Anim Male | Anim Female | Armas de la anim genérica |
|---|---|:---:|:---:|:---:|:---:|:---:|---|
| Child | 0 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Citizen | 0 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| DragonRider | 0 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| PegasusRider | 0 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Tester | 0 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| AThief | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Archer | 1 | ✅ | ✅ | ✅ | ⬜ | ⬜ | Bow |
| Armour | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| BThief | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| BThief_Lara | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Ballistae | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Bandit | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Barbarian | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Bard | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Cavalier | 1 | ✅ | ✅ | ✅ | ⬜ | ⬜ | Javelin, Lance, MagicSword, Sword |
| CavalierA | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| CavalierB | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| CavalierL | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| CavalierS | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| DragonKnight | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Fighter | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| LightPriestess | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| LoptoMage | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| LordLeaf | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| LordSeliph | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Mage | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| PegasusKnight | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Pirate | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| PreHero | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Priest | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Priestess | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Princess | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Soldier | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Swordfighter | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Troubadour | 1 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| APaladin | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| BPaladin | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Berserker | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Bishop | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| BowKnight | 2 | ✅ | ✅ | ✅ | ⬜ | ⬜ | Bow |
| Dancer | 2 | ✅ | ✅ | ✅ | ⬜ | ⬜ | MagicSword, Refresh, Sword |
| Dancer_Lara | 2 | ✅ | ✅ | ✅ | ⬜ | ⬜ | MagicSword, Refresh, Sword |
| DarkSage | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| DragonMaster | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| DukeKnight | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| FalconKnight | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| ForrestKnight | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| General | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| GreatKnight | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Hero | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| HighPriest | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| HighPriestess | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| LordKnight | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| MageFighter | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| MageKnight | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| MasterKnight | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Rogue | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Sage | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Sniper | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Swordmaster | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Warrior | 2 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Baron | 3 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| DarkBishop | 3 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| DarkPrince | 3 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Emperor | 3 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |
| Queen | 3 | ✅ | ✅ | ⬜ | ⬜ | ⬜ | — |

## Personajes — retrato y combat anim propia (PRF)

| Personaje | Clase | Gén | Retrato | Map sprite propio | Combat anim propia | Armas de su anim |
|---|---|:---:|:---:|:---:|:---:|---|
| AlvaFE4 | APaladin | M | ✅ | ⬜ | ⬜ | — |
| Eldigan | APaladin | M | ✅ | ⬜ | ⬜ | — |
| Eva | APaladin | M | ✅ | ⬜ | ⬜ | — |
| Eve | APaladin | M | ✅ | ⬜ | ⬜ | — |
| Dew | AThief | M | ✅ | ⬜ | ⬜ | — |
| Lifis | AThief | M | ✅ | ⬜ | ⬜ | — |
| Jamke | Archer | M | ✅ | ⬜ | ✅ | Bow |
| Ronan | Archer | M | ✅ | ⬜ | ✅ | Bow |
| Tanya | Archer | F | ✅ | ⬜ | ✅ | Bow |
| Arden | Armour | M | ✅ | ⬜ | ⬜ | — |
| Weissman | Armour | M | ✅ | ⬜ | ⬜ | — |
| Lara | BThief_Lara | F | ✅ | ⬜ | ⬜ | — |
| Marty | Bandit | M | ✅ | ⬜ | ⬜ | — |
| Lewyn | Bard | M | ⬜ | ⬜ | ⬜ | — |
| Chagall | Baron | M | ✅ | ⬜ | ⬜ | — |
| Raydrik | Baron | M | ✅ | ⬜ | ⬜ | — |
| Clement | Bishop | M | ✅ | ⬜ | ⬜ | — |
| Alec | Cavalier | M | ✅ | ⬜ | ⬜ | — |
| Noish | Cavalier | M | ✅ | ⬜ | ⬜ | — |
| NordionGuard | Cavalier | M | ⬜ | ⬜ | ⬜ | — |
| Lex | CavalierA | M | ✅ | ⬜ | ⚠️ | Axe, HandAxe |
| Midir | CavalierB | M | ✅ | ⬜ | ⬜ | — |
| Finn | CavalierL | M | ⬜ | ⬜ | ⬜ | — |
| Beowulf | CavalierS | M | ✅ | ⬜ | ⬜ | — |
| Coirpre | Child | M | ✅ | ⬜ | ⬜ | — |
| Sylvia | Dancer | M | ✅ | ⬜ | ⬜ | — |
| Sandima | DarkSage | M | ✅ | ⬜ | ⬜ | — |
| Elliot | DukeKnight | M | ✅ | ⬜ | ⬜ | — |
| Quan | DukeKnight | M | ✅ | ⬜ | ⬜ | — |
| Zain | DukeKnight | M | ✅ | ⬜ | ⬜ | — |
| DiMaggio | Fighter | M | ✅ | ⬜ | ⬜ | — |
| Gerrard | Fighter | M | ✅ | ⬜ | ⬜ | — |
| Halvan | Fighter | M | ✅ | ⬜ | ⬜ | — |
| Osian | Fighter | M | ✅ | ⬜ | ⬜ | — |
| Voltz | ForrestKnight | M | ✅ | ⬜ | ⬜ | — |
| Boldor | General | M | ✅ | ⬜ | ⬜ | — |
| Lobos | General | M | ✅ | ⬜ | ⬜ | — |
| Macbeth | General | M | ✅ | ⬜ | ⬜ | — |
| Phillip | General | M | ✅ | ⬜ | ⬜ | — |
| Claude | HighPriest | M | ✅ | ⬜ | ⬜ | — |
| Deirdre | LightPriestess | F | ✅ | ⬜ | ⬜ | — |
| Sigurd | LordKnight | M | ✅ | ⬜ | ⚠️ | Javelin, Lance, MagicSword, Sword |
| Leif | LordLeaf | M | ✅ | ⬜ | ⚠️ | MagicSword, Sword |
| Azel | Mage | M | ✅ | ⬜ | ⬜ | — |
| Tailtiu | Mage | M | ✅ | ⬜ | ⬜ | — |
| Erinys | PegasusKnight | M | ✅ | ⬜ | ⬜ | — |
| FakeHolyn | PreHero | M | ✅ | ⬜ | ⬜ | — |
| Holyn | PreHero | M | ✅ | ⬜ | ⬜ | — |
| Aideen | Priestess | F | ✅ | ⬜ | ⬜ | — |
| Safy | Priestess | F | ✅ | ⬜ | ⬜ | — |
| Lachesis | Princess | M | ✅ | ⬜ | ✅ | MagicStaff, MagicSword, Sword |
| Arvis | Sage | M | ⬜ | ⬜ | ⬜ | — |
| Brigid | Sniper | F | ✅ | ⬜ | ⬜ | — |
| Ayra | Swordfighter | F | ✅ | ⬜ | ⬜ | — |
| Mareeta | Swordfighter | F | ✅ | ⬜ | ⬜ | — |
| Shiva | Swordfighter | M | ✅ | ⬜ | ⬜ | — |
| Eyvel | Swordmaster | F | ✅ | ⬜ | ⬜ | — |
| Ethlyn | Troubadour | F | ✅ | ⬜ | ⬜ | — |
| Nanna | Troubadour | F | ✅ | ⬜ | ⬜ | — |
| Bucks | Warrior | M | ✅ | ⬜ | ⬜ | — |
| Dagdar | Warrior | M | ✅ | ⬜ | ⬜ | — |
| Gandolf | Warrior | M | ✅ | ⬜ | ⬜ | — |
| GenoaGuard | Warrior | M | ⬜ | ⬜ | ⬜ | — |
| Kinbois | Warrior | M | ✅ | ⬜ | ⬜ | — |

## ⚠️ Problemas detectados (el juego no encuentra estos anims)

- Clase **`LordKnight`**: `combat_anim_nid = "Lord Knight"` tiene un **espacio** → el resolver busca una carpeta con espacio que nunca existirá. Quitar el espacio.
- **`Lex`** (CavalierA): anim en `assets/combat_anims/AxeKnight_Lex/` pero `combat_anim_nid="AxeCavalier"` → el resolver busca `AxeCavalier_Lex` (no existe). Renombrar la carpeta a `AxeCavalier_Lex` **o** corregir `combat_anim_nid`.
- **`Sigurd`** (LordKnight): anim en `assets/combat_anims/LordKnight_Sigurd/` pero `combat_anim_nid="Lord Knight"` → el resolver busca `Lord Knight_Sigurd` (no existe). Renombrar la carpeta a `Lord Knight_Sigurd` **o** corregir `combat_anim_nid`.
- **`Leif`** (LordLeaf): anim en `assets/combat_anims/Prince_Leif/` pero `combat_anim_nid="LordLeaf"` → el resolver busca `LordLeaf_Leif` (no existe). Renombrar la carpeta a `LordLeaf_Leif` **o** corregir `combat_anim_nid`.
