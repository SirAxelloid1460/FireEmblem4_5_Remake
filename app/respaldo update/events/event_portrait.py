...
from app.constants import PORTRAIT_WIDTH, ALT_PORTRAIT_WIDTH, PORTRAIT_HEIGHT, COLORKEY, EDITION
...
class EventPortrait():
    ...
    if EDITION == 'Axelloid1460':
        width, height = ALT_PORTRAIT_WIDTH, PORTRAIT_HEIGHT
        halfblink = (96, 64, 32, 16)
        fullblink = (96, 80, 32, 16)
        chibi_coords = (96, 0, 48, 64)
    else:
    ...
    def draw_chibi(self, surf):
        image = self.chibi.copy()
        if self.mirror:
            image = engine.flip_horiz(image)
        ...
        position = self.position[0] + 24, self.position[1] + 16
